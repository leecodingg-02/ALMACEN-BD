...
async function importarBaseDeDatos() {
  ...
  try {
    ...
    console.log('/// Ejecutando sentencias SQL (creando tablas e insertando datos)...');
    await conexion.query(contenidoSql);

    console.log(':D ¡Base de datos bd_almacen_1 importada con éxito!');
    console.log('💡 Ya puedes iniciar tu servidor con: npm run server');
  } catch (error) {
    console.error('X Error durante la importación:');
